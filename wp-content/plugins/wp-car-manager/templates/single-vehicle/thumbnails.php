<script type="text/javascript">
    $( document ).ready(function() {
        var thumbs = $(".wpcm-thumbnails").find('a.zoom');
        thumbs.click(function (){
            var imgThumbs = $(this).find('attachment-wpcm_vehicle_thumbnail');
            var filename = imgThumbs.children(0).context.childNodes["0"].alt;
            var str = imgThumbs.children(0).context.childNodes["0"].src;
            var base = new String(str).substring(str.lastIndexOf('/') + 1);
            var res = str.replace(base, "");
            $('.wpcm-main-image')["0"].childNodes["0"].src=res+filename+"-600x400.jpg";
            $('.wpcm-main-image')['0'].href=res+filename+"-600x400.jpg";
