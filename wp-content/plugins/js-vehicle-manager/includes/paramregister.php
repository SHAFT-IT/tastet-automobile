...
function jsvm_generate_rewrite_rules(&$rules, $rule){
    ...
    foreach($rules AS $key => $value){
        if(strstr($key, $rule)){
            ...
            $_key = $newkey . '/(vehicle-search|vehicle-by-cities|seller-by-cities|vehicle-by-conditions|vehicle-by-types|vehicle-detail|vehicle-by-makes|shortlisted-vehicles|vehicle-alerts|pricing|'.$credit_logs.'|'.$packages.'|'.$edit_profile.'|'.$my_profile.'|sellers|seller|'.$purchase_history.'|'.$registration.'|'.$login.'|'.$stats.'|compare|add-vehicle|my-vehicles|print-vehicle|vehicle-pdf|vehicles|vehicle)(/[^/]*)?(/[^/]*)?(/[^/]*)?/?$';
...
function jsvm_root_rewrite_rules( $rules ) {
        ...
        $rules['('.$edit_profile.'|vm-vehicle-by-cities|vm-seller-by-cities|vm-vehicle-search|vm-vehicle-by-conditions|vm-vehicle-detail|vm-vehicle-by-types|vm-vehicle-by-makes|vm-shortlisted-vehicles|vm-vehicle-alerts|vm-pricing|'.$credit_logs.'|'.$packages.'|'.$my_profile.'|vm-sellers|vm-seller|'.$purchase_history.'|'.$registration.'|'.$login.'|'.$stats.'|vm-compare|vm-add-vehicle|vm-my-vehicles|vm-print-vehicle|vm-vehicle-pdf|vm-vehicles|vm-vehicle)(/[^/]*)?(/[^/]*)?(/[^/]*)?/?$'] = 'index.php?page_id='.$pageid.'&jsvmlayout=$matches[1]&jsvm1=$matches[2]&jsvm2=$matches[3]&jsvm3=$matches[4]';
        return $rules;
}
...
function jsvm_redirect_canonical($redirect_url, $requested_url) {
    ...
    if(is_home() || is_front_page()){
        $array = array('/vm-vehicle-by-cities','/vm-seller-by-cities','/vm-vehicle-by-types','/vm-vehicle-by-makes','/vm-vehicles','/vm-vehicle','/vm-shortlisted-vehicles','/vm-vehicle-alerts','/vm-pricing','/vm-credit-logs','/vm-vehicle-credit-logs','/vm-packages','/vm-vehicle-packages'
            ,'/vm-edit-profile','/vm-vehicle-edit-profile','/vm-my-profile','/vm-vehicle-my-profile','/vm-sellers','/vm-seller','/vm-purchase-history','/vm-vehicle-purchase-history','/vm-registration','/vm-vehicle-registration','/vm-login','/vm-vehicle-login','/vm-compare','/vm-add-vehicle','/vm-my-vehicles','/vm-print-vehicle','/vm-vehicle-pdf');
        $ret = false;
