    ...
    function formCustomFieldsForSearch($field, &$i, $isadmin = 0,$search_popup=0) {
        ...
        switch ($field->userfieldtype) {
            ...
            case 'multiple':
                ...
                $array = $field->field;
                $array .= '[]';
                $html .= JSVEHICLEMANAGERformfield::select($array, $comboOptions, $value, __('Select', 'js-vehicle-manager') . ' ' . $field->fieldtitle, array('data-validation' => $cssclass, 'multiple' => 'multiple', 'data-placeholder'=>__('Select', 'js-vehicle-manager') . ' ' . $field->fieldtitle));
                break;
        }
        if($isadmin == 1){
            return $html;
    ...
    function showCustomFields($field, $fieldfor, $params,$seller = 0) {
        ...
        if($fieldfor == 1){ // For Detail
            ...
            if($field->userfieldtype=='file'){
               if($fvalue !=null){
                    if($seller == 0){
                        $path = admin_url("?page=vehicle&action=jsvmtask&task=downloadbyname&id=".jsvehiclemanager::$_data['objectid']."&name=".$fvalue);
                    }else{
                        $path = admin_url("?page=vehicle&action=jsvmtask&task=downloadbyname&id=".jsvehiclemanager::$_data['objectid']."&name=".$fvalue."&seller=1");
                    }
