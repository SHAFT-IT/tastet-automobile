...
class JSVEHICLEMANAGERjsvehiclemanagerModel {
    ...
    function getTodayStatsForWidget(){
        ...
        //Today 
        $query = "SELECT COUNT(id) FROM `#__js_vehiclemanager_vehicles` WHERE isfeaturedvehicle = 1 AND DATE(created) = CURDATE() AND DATE(startfeatureddate) <= CURDATE() AND DATE(endfeatureddate) >= CURDATE()";
        $db->setQuery($query);
        $data = $db->loadResult();
        $result['today_featured'] = $data;
        //Today 
        $query = "SELECT COUNT(id) FROM `#__js_vehiclemanager_vehicles` WHERE issold = 1 AND DATE(solddate) = CURDATE()";
        $db->setQuery($query);
        $data = $db->loadResult();
        $result['today_sold'] = $data;
        //Today 
        $query = "SELECT COUNT(id) FROM `#__js_vehiclemanager_vehicles` WHERE DATE(adexpiryvalue) = CURDATE()";
        $db->setQuery($query);
        $data = $db->loadResult();
    ...
    function getTotalStatsForWidget(){
        ...
        $query = "SELECT COUNT(id) FROM `#__js_vehiclemanager_vehicles` WHERE status = 1 AND DATE(adexpiryvalue) >= CURDATE() AND isfeaturedvehicle = 1 AND DATE(startfeatureddate) <= CURDATE() AND DATE(endfeatureddate) >= CURDATE()";
        $db->setQuery($query);
        $data = $db->loadResult();
        $result['feature_active_vehicle'] = $data;
        ...
        switch ($for) {
            case '1': 
                ...
                <div class="jsvehiclemanager-widget-contaner">
                    ...
                    <div class="jsvehiclemanager-widget-block mrgn-bottm">
                        <div class="jsvehiclemanager-widget-img">
                            <img src="'.jsvehiclemanager::$_pluginpath.'includes/images/widgets/featured.png'.'" />
                        </div>
                        <div class="jsvehiclemanager-widget-data">
                            <p class="heading">'.$data['today_featured'].'</p>
                            <p class="detail">'.__('Featured Vehicles','js-vehicle-manager').'</p>
                        </div>
                    </div>
                    <div class="jsvehiclemanager-widget-block mrgn-right">
                        <div class="jsvehiclemanager-widget-img">
                            <img src="'.jsvehiclemanager::$_pluginpath.'includes/images/widgets/active.png'.'" />
                        </div>
                        <div class="jsvehiclemanager-widget-data">
                            <p class="heading">'.$data['today_sold'].'</p>
                            <p class="detail">'.__('Sold Vehicles','js-vehicle-manager').'</p>
                        </div>
                    </div>
                    <div class="jsvehiclemanager-widget-block">
                        <div class="jsvehiclemanager-widget-img">
            ...
            case '4': // total stats
                ...
                <div class="jsvehiclemanager-widget-contaner">
                    ...
                    <div class="jsvehiclemanager-widget-total-block">
                        <img class="totalimage" src="'.jsvehiclemanager::$_pluginpath.'includes/images/widgets/featured-2.png'.'" />
                        <span class="total-detail">'.__('Featured Vehicles','js-vehicle-manager').'</span>                        
                        <span class="total-total">'.$data['feature_active_vehicle'].'</span>
                    </div>
                    <div class="jsvehiclemanager-widget-total-block">
    ...
    function getCurrentTheme() {
        $filepath = jsvehiclemanager::$_path . 'includes/css/site_color.php';
        ...
        jsvehiclemanager::$_data[0] = $theme;
        return $theme;
    }
    ...
    function storeServerSerailNumber($data) {
        ...
        if ($data['server_serialnumber']) {
            $query = "UPDATE  `#__js_vehiclemanager_config` SET configvalue='" . $data['server_serialnumber'] . "' WHERE configname='server_serial_number'";
            $db = new jsvehiclemanagerdb();
            if (!$db->query($query))
                return SAVE_ERROR;
            else
    ...
    function getStepTwoValidate() {
        $basepath = ABSPATH;
        JSVEHICLEMANAGERincluder::getJSModel('common')->makeDir($basepath . '/tmp');
        jsvehiclemanager::$_data['dir'] = substr(sprintf('%o', fileperms(jsvehiclemanager::$_path)), -3);

        if (!is_writable(jsvehiclemanager::$_path)) {
            jsvehiclemanager::$_data['dir'] = 0;
        }
        jsvehiclemanager::$_data['tmp_dir'] = substr(sprintf('%o', fileperms($basepath . '/tmp')), -3);
        if (!is_writable($basepath . '/tmp')) {
            jsvehiclemanager::$_data['tmp_dir'] = 0;
        }
        ...
        $db = new jsvehiclemanagerdb();
        jsvehiclemanager::$_data['create_table'] = 0;
        $db->setQuery($query);
        if ($db->query()) {
            jsvehiclemanager::$_data['create_table'] = 1;
        }
        $query = 'INSERT INTO js_test_table(id,name) VALUES (1,\'Naeem\'),(2,\'Saad\');';
        jsvehiclemanager::$_data['insert_record'] = 0;
        $db->setQuery($query);
        if ($db->query()) {
            jsvehiclemanager::$_data['insert_record'] = 1;
        }
        $query = 'UPDATE js_test_table SET name = \'Shoaib Rehmat\' WHERE id = 1;';
        jsvehiclemanager::$_data['update_record'] = 0;
        $db->setQuery($query);
        if ($db->query()) {
            jsvehiclemanager::$_data['update_record'] = 1;
        }
        $query = 'DELETE FROM js_test_table;';
        jsvehiclemanager::$_data['delete_record'] = 0;
        $db->setQuery($query);
        if ($db->query()) {
            jsvehiclemanager::$_data['delete_record'] = 1;
        }
        $query = 'DROP TABLE js_test_table;';
        jsvehiclemanager::$_data['drop_table'] = 0;
        $db->setQuery($query);
        if ($db->query()) {
            jsvehiclemanager::$_data['drop_table'] = 1;
        }
        if (jsvehiclemanager::$_data['tmp_dir'] >= 755) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
            ...
            curl_close($ch);
            fclose($fp);
            jsvehiclemanager::$_data['file_downloaded'] = 0;
            if (file_exists('../tmp/logo.png')) {
                jsvehiclemanager::$_data['file_downloaded'] = 1;
            }
        } else
            jsvehiclemanager::$_data['file_downloaded'] = 0;
        return jsvehiclemanager::$_data;
    }
    ...
    function getmyversionlist() {
        ...
        $post_data['producttype'] = JSVEHICLEMANAGERrequest::getVar('producttype', null, 'pro');
        ...
        $post_data['count'] = JSVEHICLEMANAGERincluder::getJSModel('configuration')->getCountConfig();
        $post_data['installerversion'] = JSVEHICLEMANAGERrequest::getVar('installerversion');
        $ch = curl_init();
