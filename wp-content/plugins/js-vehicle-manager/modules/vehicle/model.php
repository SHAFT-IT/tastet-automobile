...
class JSVEHICLEMANAGERvehicleModel {
        ...
        */
        ...
        if($datafor == 1){
            ...
            switch ($isgfcombo) {
                ...
                break;
                case '2':
                    $inquery .= " AND veh.isfeaturedvehicle = 1 AND DATE(veh.startfeatureddate) <= '".$curdate."' AND DATE(veh.endfeatureddate) >= '".$curdate."' ";
                break;
            }
        }else{ // For Queue
            switch ($isgfcombo) {
                case '1':
                    $inquery = " WHERE (veh.status = 0 OR veh.isfeaturedvehicle = 0) ";
                break;
                case '2':
                    $inquery = " WHERE veh.isfeaturedvehicle = 0 ";
                break;
                default:
                    $inquery = " WHERE (veh.status = 0 OR veh.isfeaturedvehicle = 0) ";
                break;
        ...
        //data
        $query = "SELECT veh.price, veh.uid, veh.created,  veh.cityfuelconsumption, veh.highwayfuelconsumption, veh.enginecapacity, veh.mileages, veh.status, veh.adexpiryvalue, veh.regcity, veh.loccity, veh.isfeaturedvehicle, veh.startfeatureddate, veh.endfeatureddate ,
    ...
    function getVehicleExpiryStatus($vehicleid) {
        ...
        if($soldflag == 0){
            $soldquery = " AND vehicle.issold = 0 ";
        }else{
            $soldquery = " ";
        }
    ...
    function getSortingForListing($ordering_ajax = '' , $sorting_ajax = '') {
        ...
        $sorting = JSVEHICLEMANAGERrequest::getVar('cmsorting');
        if($ordering_ajax != ''){
    ...
    function getVehicles($ajax_flag = 0 ){
        ...
        jsvehiclemanager::$_data['total'] = $total;
        jsvehiclemanager::$_data[1] = JSVEHICLEMANAGERpagination::getPagination($total);
        $featuredconfig = JSVEHICLEMANAGERincluder::getJSModel('configuration')->getConfigByFor('featuredvehicles');
        $nooffeaturedvehicles = $featuredconfig['no_of_featured_vehicles_in_vehicle_listing'];
        $featuredvehicles = array();//to handle if featured vehicles disable
        if( $featuredconfig['show_featured_vehicles_in_vehicles_listing'] && is_numeric($nooffeaturedvehicles) && $nooffeaturedvehicles != 0){
            // featured vehicles
            $fpagenum = JSVEHICLEMANAGERrequest::getVar('pagenum');
            $offset = ($fpagenum != 0) ? ($nooffeaturedvehicles * $fpagenum) : 0;
            $query = "SELECT veh.id, veh.price, veh.created, veh.cityfuelconsumption, veh.highwayfuelconsumption, veh.enginecapacity, veh.mileages, veh.status, veh.adexpiryvalue, veh.regcity, veh.loccity,
                        make.title AS maketitle, model.title AS modeltitle, cond.title AS conditiontitle, transmission.title AS transmissiontitle, fueltype.title AS fueltypetitle,
                        mileage.symbol AS mileagesymbol, currency.symbol AS currencysymbol,veh.id AS vehicleid,modelyear.title AS modelyeartitle,veh.isfeaturedvehicle,veh.stocknumber
                        ,veh.exteriorcolor,veh.params,vehicleimage.filename as imagename,vehicleimage.file as filepath,veh.issold,cond.color AS conditioncolor
                        ,(SELECT COUNT(vim.id) FROM `#__js_vehiclemanager_vehicleimages` AS vim WHERE vim.vehicleid=veh.id ) AS totalimages,veh.endfeatureddate
                        ...
                        LEFT JOIN `#__js_vehiclemanager_modelyears` AS modelyear ON modelyear.id = veh.modelyearid
                        LEFT JOIN `#__js_vehiclemanager_vehicleimages` AS vehicleimage ON vehicleimage.vehicleid = veh.id AND vehicleimage.isdefault = 1
                        WHERE veh.status = 1 AND DATE(veh.adexpiryvalue) >= CURDATE() AND veh.isfeaturedvehicle = 1
                        AND  DATE(veh.endfeatureddate) >= CURDATE() ";
            $query = $query.$soldquery;
            $query .= $inquery;
            $query .= jsvehiclemanager::$_data['ordering_query'];
            $query.= " LIMIT $offset , $nooffeaturedvehicles";
            $db->setQuery($query);
            $featuredvehicles = $db->loadObjectList();
        }
        //vehicles
        $query = "SELECT veh.id, veh.price, veh.created, veh.cityfuelconsumption, veh.highwayfuelconsumption, veh.enginecapacity, veh.mileages, veh.status, veh.adexpiryvalue, veh.regcity, veh.loccity,
                    make.title AS maketitle, model.title AS modeltitle, cond.title AS conditiontitle, transmission.title AS transmissiontitle, fueltype.title AS fueltypetitle,
                    mileage.symbol AS mileagesymbol, currency.symbol AS currencysymbol,veh.id AS vehicleid,modelyear.title AS modelyeartitle,veh.isfeaturedvehicle,veh.stocknumber
                    ,veh.exteriorcolor,veh.params,vehicleimage.filename as imagename,vehicleimage.file as filepath,veh.issold,veh.endfeatureddate
