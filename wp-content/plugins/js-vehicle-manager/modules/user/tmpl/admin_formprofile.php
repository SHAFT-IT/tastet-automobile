...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        jQuery("a#jsvm_userpopup").click(function (e) {
            ...
            jsvm_showloading();
            jQuery.post(ajaxurl, {action: 'jsvehiclemanager_ajax', jsvmme: 'user', task: 'getUserListAjax'}, function (data) {
                if (data) {
        ...
        jQuery(document).delegate('form#jsvm_userpopupsearch', 'submit', function (e) {
            ...
            var ajaxurl = "<?php echo admin_url('admin-ajax.php') ?>";
            jQuery.post(ajaxurl, {action: 'jsvehiclemanager_ajax', jsvmme: 'user', task: 'getUserListAjax', name: name, uname: username, email: emailaddress, listfor: 1}, function (data) {
                if (data) {
                    jQuery("span#jsvm_popup_title").html(jQuery("input#jsvm_user-popup-title-text").val());
                    jQuery("div#jsvm_popup-record-data").html(data);
                    setUserLink();
    ...
    function updateuserlist(pagenum) {
        var ajaxurl = "<?php echo admin_url('admin-ajax.php') ?>";
        jQuery.post(ajaxurl, {action: 'jsvehiclemanager_ajax', jsvmme: 'user', task: 'getUserListAjax', userlimit: pagenum}, function (data) {
            if (data) {
                jQuery("div#jsvm_popup-record-data").html("");
                jQuery("span#jsvm_popup_title").html(jQuery("input#jsvm_user-popup-title-text").val());
                jQuery("div#jsvm_popup-record-data").html(data);
        ...
        jQuery("#cityid").tokenInput(vehicleArray, {
            ...
            onAdd: function(item) {
                ...
                if ($newtyped_cities == 1) { ?>
                    ...
                    var ajaxurl = "<?php echo admin_url('admin-ajax.php') ?>";
                            jQuery.post(ajaxurl, {action: 'jsvehiclemanager_ajax', jsvehiclemanagerme: 'city', task: 'savetokeninputcity', citydata: jQuery("tester").text()}, function(data){
                            if (data){
                            try {
