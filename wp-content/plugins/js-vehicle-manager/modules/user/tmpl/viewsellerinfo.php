...
<div class="jsvehiclemanager_cm-seller-wrap">
    ...
    <div class="row jsvehiclemanager_cm-margin jsvehiclemanager_cm-message-map-wrap">
        <?php if($mapshow == true){ ?>
            <div class="col-sm-6 jsvehiclemanager_cm-map jsvehiclemanager_cm-map-full-width">
...
<?php if(JSVEHICLEMANAGERincluder::getObjectClass('user')->isguest() && jsvehiclemanager::$_data['showcaptcha'] == 1) {
    wp_enqueue_script( 'recaptchaAPI','https://www.google.com/recaptcha/api.js?onload=CaptchaCallback&render=explicit');
}
 ?>
<?php if($mapshow == true){
    wp_enqueue_script( 'mapAPI','https://maps.googleapis.com/maps/api/js?key='.$google_api_key);
}
?>
