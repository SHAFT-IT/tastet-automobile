...
class JSVEHICLEMANAGERUserModel {
    ...
    function getAllprofile() {
        ...
        //Data
        $query = "SELECT id, name, cell, phone, email, weblink, status, issocial, photo, cityid, latitude, longitude
    ...
    function getAllSellerInfos() {
        ...
        jsvehiclemanager::$_data['total'] = $total;
        jsvehiclemanager::$_data[1] = JSVEHICLEMANAGERpagination::getPagination($total);
        $query = "SELECT user.id, user.name, user.cell, user.phone, user.email, user.weblink, user.status, user.issocial, user.photo, user.cityid, user.params
                        FROM `#__js_vehiclemanager_users` AS user
                        JOIN `#__js_vehiclemanager_vehicles` As veh ON veh.uid = user.id
                        WHERE user.status = 1";
        $query .=   $inquery;
    ...
    function getProfileByUid($id) {
        ...
        if(JSVEHICLEMANAGERincluder::getObjectClass('user')->isSellerCheck($id)){
            ...
            $query = "SELECT COUNT(id) FROM `#__js_vehiclemanager_vehicles` WHERE uid = " . $id. " AND isfeaturedvehicle = 1 ";
            $db->setQuery($query);
            jsvehiclemanager::$_data['featuredvehicles'] = $db->loadResult();
    ...
    function getUserStats($uid){
        ...
        $query = "SELECT COUNT(id) FROM `#__js_vehiclemanager_vehicles` WHERE uid = " . $uid. " AND isfeaturedvehicle = 1 ";
        $db->setQuery($query);
        jsvehiclemanager::$_data['featuredvehicles'] = $db->loadResult();
        ...
        $query = "SELECT COUNT(id) FROM `#__js_vehiclemanager_vehicles` WHERE uid = " . $uid. " AND isfeaturedvehicle = 1 AND adexpiryvalue < CURDATE() ";
        $db->setQuery($query);
        jsvehiclemanager::$_data['featuredexpiredvehicles'] = $db->loadResult();
    }
