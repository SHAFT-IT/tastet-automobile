...
class JSVEHICLEMANAGERPostinstallationModel {
    ...
    function installSampleData($insert_sample_data, $menu, $insert_template_data) {
        ...
        global $wpdb;
        if($insert_sample_data == 1 || $insert_template_data == 1) {
        ...
        global $wpdb;
        $basepath = ABSPATH;
        if ($insert_sample_data == 1) {// eihter 4 vehicles or 26 vehicles
        ...
        }elseif ($insert_template_data == 1) { // insert template and plugin data
           // sampla data containin images of 26 vehicles
            ...
            curl_setopt($ch, CURLOPT_URL, "http://setup.joomsky.com/jsvehiclemanager/pro/plugin/sample_data/js_car_manager/cm_sample_data.zip");
            $fp = fopen($basepath . "/tmp/cm_sample_data.zip", "w+");
            ...
            // for testing purposes this code is added
            // if (file_exists( jsvehiclemanager::$_path. "/includes/sampledata/cmcm_sample_data.zip")) {
            //     require_once jsvehiclemanager::$_path . '/includes/lib/pclzip.lib.php';
            //     $archive = new PclZip(jsvehiclemanager::$_path. "/includes/sampledata/cmcm_sample_data.zip");
            //      $v_list = $archive->extract($wp_upload_dir["basedir"]."/jsvehiclemanager_data/");
            // }
            ...
            $this->importTemplateSampleData('f');
    ...
    function importTemplateSampleData($flag){
        if($flag != 'f'){
            ...
            $wp_upload_dir = wp_upload_dir();
            // sample data zip extraction
            // if (file_exists( jsvehiclemanager::$_path. "/includes/sample-data1.zip")) {
            //     require_once jsvehiclemanager::$_path . '/includes/lib/pclzip.lib.php';
            //     $archive = new PclZip(jsvehiclemanager::$_path. "/includes/sample-data1.zip");
            //     $v_list = $archive->extract($wp_upload_dir["basedir"]."/jsvehiclemanager_data/");
            // // }
            //     $ch = curl_init();
            //     curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
            ...
            //     fclose($fp);
                ...
                // Check for the js support ticket
            // template data
            // Check for the rev slider
                ...
                if($error_revslider == false){
                    //Download Rev slider sampledata
                    // $ch = curl_init();
                    // curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
                    // curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
                    // curl_setopt($ch, CURLOPT_URL, "http://setup.joomsky.com/jsvehiclemanager/pro/plugin/sample_data/revolution_slider/rs_sample_data.zip");
                    // $fp = fopen($basepath . "/tmp/rs_sample_data.zip", "w+");
                    // curl_setopt($ch, CURLOPT_FILE, $fp);
                    // curl_setopt($ch, CURLOPT_TIMEOUT, 0);
                    // curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 0);
                    // curl_exec($ch);
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["home"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["home"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["home"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["home"], "cm_show_header", 2);
                // Home 1
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["home1"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["home1"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["home1"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["home1"], "cm_show_header", 2);
                // home 2
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["home2"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["home2"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["home2"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["home2"], "cm_show_header", 2);
                // home 3
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["home3"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["home3"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["home3"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["home3"], "cm_show_header", 2);
                // home 4
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["home4"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["home4"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["home4"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["home4"], "cm_show_header", 2);
                // home 5
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["home5"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["home5"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["home5"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["home5"], "cm_show_header", 2);
                $wp_upload_dir = wp_upload_dir();
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["pricing_table"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["pricing_table"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["pricing_table"], "cm_show_header", 1);
                update_post_meta($cm_pages["pricing_table"], "_wp_page_template", $new_page_template);

                // Thank you
                $new_page_title = "Thank you";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="20"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["thank_you"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["thank_you"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["thank_you"], "cm_show_header", 1);
                update_post_meta($cm_pages["thank_you"], "_wp_page_template", $new_page_template);


                // vehicles
                $new_page_title = "Vehicles";
                $new_page_content = '[vc_row][vc_column][cm_popular_vehicles style="1" scrollstyle="1" speed="1" posts_per_page="10"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_manager_pages page="1"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["vehicles"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["vehicles"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["vehicles"], "cm_show_header", 1);
                $image = $wp_upload_dir["baseurl"]."/2017/01/vehicles_header.png";
                update_post_meta($cm_pages["vehicles"], "cm_headerimage_url", $image);
                update_post_meta($cm_pages["vehicles"], "_wp_page_template", $new_page_template);

                // vehicle detail
                $new_page_title = "Vehicle Detail";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="7"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["vehicledetail"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["vehicledetail"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["vehicledetail"], "cm_show_header", 1);
                $image = $wp_upload_dir["baseurl"]."/2017/01/vehicles_header.png";
                update_post_meta($cm_pages["vehicledetail"], "cm_headerimage_url", $image);
                update_post_meta($cm_pages["vehicledetail"], "_wp_page_template", $new_page_template);

                

                // vehicles by city
                $new_page_title = "Vehicles By City";
                $new_page_content = '[vc_row full_width="stretch_row_content_no_spaces"][vc_column][cm_vehicle_gmap mapheight="450" jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_manager_pages page="9"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["vehicles_by_city"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["vehicles_by_city"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["vehicles_by_city"], "cm_show_header", 1);
                update_post_meta($cm_pages["vehicles_by_city"], "_wp_page_template", $new_page_template);
                
                // Sellers by city
                $new_page_title = "Sellers By City";
                $new_page_content = '[vc_row full_width="stretch_row_content_no_spaces"][vc_column][cm_vehicle_gmap jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" mapheight="450" mapfor="2"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_manager_pages page="21"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["sellers_by_city"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["sellers_by_city"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["sellers_by_city"], "cm_show_header", 1);
                update_post_meta($cm_pages["sellers_by_city"], "_wp_page_template", $new_page_template);
                

                // vehicles by make
                $new_page_title = "Vehicles By Make";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="11"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["vehicles_by_make"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["vehicles_by_make"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["vehicles_by_make"], "cm_show_header", 1);
                update_post_meta($cm_pages["vehicles_by_make"], "_wp_page_template", $new_page_template);
                // vehicles by type
                $new_page_title = "Vehicles By Type";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="10"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["vehicles_by_type"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["vehicles_by_type"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["vehicles_by_type"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["vehicles_by_type"], "cm_show_header", 1);
                // Seller List
                $new_page_title = "Sellers List";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="12"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["seller_list"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["seller_list"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["seller_list"], "cm_show_header", 1);
                update_post_meta($cm_pages["seller_list"], "_wp_page_template", $new_page_template);
                // Search Vehicles
                $new_page_title = "Search";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="2"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                        "post_title" => $new_page_title,
                        "post_name" => "search-vehicles",
                        "post_content" => $new_page_content,
                        "post_status" => "publish",
                        "post_author" => 1,
                        "post_parent" => 0,
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["search_vehicles"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["search_vehicles"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["search_vehicles"], "cm_show_header", 1);
                update_post_meta($cm_pages["search_vehicles"], "_wp_page_template", $new_page_template);
                // My Profile
                $new_page_title = "My Profile";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="5"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["my_profile"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["my_profile"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["my_profile"], "cm_show_header", 1);
                update_post_meta($cm_pages["my_profile"], "_wp_page_template", $new_page_template);
                // Add Vehicle
                $new_page_title = "Add Vehicle";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="3"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["add_vehicle"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["add_vehicle"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["add_vehicle"], "cm_show_header", 1);
                update_post_meta($cm_pages["add_vehicle"], "_wp_page_template", $new_page_template);
                // My Vehicles
                $new_page_title = "My Vehicles";
                $new_page_content = '[vc_row][vc_column][cm_vehicle_manager_pages page="4"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["my_vehicles"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["my_vehicles"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["my_vehicles"], "cm_show_header", 1);
                $image = $wp_upload_dir["baseurl"]."/2017/01/myvehicles_header.png";
                update_post_meta($cm_pages["my_vehicles"], "_wp_page_template", $new_page_template);
                // News & Rumors
                $new_page_title = "News & Rumors";
                $new_page_content = '[vc_row][vc_column][cm_news_and_rumors style="2" posts_per_page="2"][/vc_column][/vc_row]';
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["news_and_rumors"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["news_and_rumors"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["news_and_rumors"], "cm_show_header", 1);
                $image = $wp_upload_dir["baseurl"]."/2017/01/newsandrumors_header.png";
                update_post_meta($cm_pages["news_and_rumors"], "cm_headerimage_url", $image);
                update_post_meta($cm_pages["news_and_rumors"], "_wp_page_template", $new_page_template);
                // FAQ
                $new_page_title = "FAQ";
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["faq"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["faq"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["faq"], "cm_show_header", 1);
                update_post_meta($cm_pages["faq"], "cm_headerimage_url", $image);
                update_post_meta($cm_pages["faq"], "_wp_page_template", $new_page_template);
                // Our Team
                $new_page_title = "Our Team";
                $new_page_content = '[vc_row][vc_column][cm_team_memebers per_row="3" posts_per_page="3" style="3"][/vc_column][/vc_row][vc_row][vc_column][cm_team_memebers per_row="4" posts_per_page="4" heading="Other Team Members" style="4"][/vc_column][/vc_row]';
                $new_page_template = "templates/template-fullwidth.php";
                $page_check = get_page_by_title($new_page_title);
                $new_page = array(
                        "post_type" => "page",
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["ourteam"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["ourteam"] = wp_insert_post($new_page);
                }
                update_post_meta($cm_pages["ourteam"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["ourteam"], "cm_show_header", 1);
                // Contact Us
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["contact_us"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["contact_us"] = wp_insert_post($new_page);        
                }
                update_post_meta($cm_pages["contact_us"], "_wp_page_template", $new_page_template);
                update_post_meta($cm_pages["contact_us"], "cm_show_header", 1);
                // Blog Page
                $new_page_title = "Blog";
                $new_page_content = "";
                $page_check = get_page_by_title($new_page_title);
                ...
                if(!isset($page_check->ID)){
                    $cm_pages["blog"] = wp_insert_post($new_page);
                }else{
                    $new_page["post_title"] = "Car Manager ".$new_page_title;
                    $cm_pages["blog"] = wp_insert_post($new_page);
                }
                update_option("page_for_posts", $cm_pages["blog"]);
                $image = $wp_upload_dir["baseurl"]."/2017/01/blog_header.png";
                update_post_meta($cm_pages["blog"], "cm_headerimage_url", $image);
                // Update home page contents
                //Home
                $new_page_content = '[vc_row][vc_column][cm_slideshow jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" vehicletype="2" slidewidth="1170" slideheight="690" posts_per_page="20"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_search jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" style="3"][/vc_column][/vc_row][vc_row][vc_column][cm_featured_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="1" speed="4" posts_per_page="10"][/vc_column][/vc_row][vc_row][vc_column][cm_carmanagerstats icon4="fa fa-car"][/vc_column][/vc_row][vc_row][vc_column][cm_twoColorBoxLinksstyle2 box1heading="Looking for car ?" box1link="url:'.urlencode(get_the_permalink($cm_pages["vehicles"])).'|title:SEARCH%20YOUR%20DREAM%20CAR|" box2link="url:'.urlencode(get_the_permalink($cm_pages["add_vehicle"])).'|title:SELL%20YOUR%20CAR|"][/vc_column][/vc_row][vc_row][vc_column][cm_buy_now_with_shortdescription title="Lorem Ipsum is text of the printing and typesetting industry." link="url:'.urlencode(get_the_permalink($cm_pages["add_vehicle"])).'|title:Post%20Vehicle|"][/vc_column][/vc_row][vc_row full_width="stretch_row" css=".vc_custom_1477026076936{background-color: #f0f0f0 !important;}"][vc_column][cm_popular_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="1" scrollstyle="1" speed="1" posts_per_page="10"][/vc_column][/vc_row][vc_row][vc_column][cm_newest_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="1" posts_per_page="10" icon="fa fa-newspaper-o"][/vc_column][/vc_row][vc_row][vc_column][cm_4count_box style="4" count1="787" count2="46" count3="519" count4="196"][/vc_column][/vc_row][vc_row][vc_column][cm_buy_now_with_shortdescription title="Lorem Ipsum is simply dummy text of the typesetting."link="'.urlencode(get_the_permalink($cm_pages["vehicles"])).'|title:Newest%20Vehicles||"][/vc_column][/vc_row][vc_row][vc_column][cm_news_and_rumors style="3"][/vc_column][/vc_row][vc_row][vc_column][cm_testimonial style="2" heading="Happy Clients" testimonialstext="Feedback of Our Happy Clients."][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_types jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" posts_per_page="10" scrollstyle="1" displaystyle="2" displaynumber="1" speed="1" heading="Vehicle by types" description="Vehicle types are listed here lorem iosum"][/vc_column][/vc_row][vc_row][vc_column][cm_buy_now_with_shortdescription title="Lorem Ipsum is simply dummy text of the printing and." link="url:'.urlencode(get_the_permalink($cm_pages["add_vehicle"])).'|title:Post%20|"][/vc_column][/vc_row]';
                $my_post = array(
                    "ID"           => $cm_pages["home"],
                    "post_content" => $new_page_content,
                );
                ...
                wp_update_post( $my_post );
                //Home 2
                $new_page_content = '[vc_row full_width="stretch_row_content_no_spaces"][vc_column][cm_vehicle_gmap jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" mapheight="450"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_search jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" style="4"][/vc_column][/vc_row][vc_row][vc_column][cm_newest_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="3" posts_per_page="10"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_types jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" posts_per_page="10" scrollstyle="1" speed="1" heading="Vehicle by types" description="Vehicle types are listed here"][/vc_column][/vc_row][vc_row][vc_column][cm_4count_box style="3" count1="387" count2="46" count3="919" count4="196"][/vc_column][/vc_row][vc_row full_width="stretch_row" css=".vc_custom_1482236697714{background-color: #f7f7f8 !important;}"][vc_column][cm_popular_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="1" scrollstyle="1" speed="1" posts_per_page="10"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_makes jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" style="1" scrollstyle="1" posts_per_page="10" speed="1" heading="Vehicle Makes" description="Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer"][/vc_column][/vc_row]';
                $my_post = array(
                    "ID"           => $cm_pages["home2"],
                    "post_content" => $new_page_content,
                );
                ...
                wp_update_post( $my_post );
                //Home 4
                $new_page_content = '[vc_row full_width="stretch_row_content_no_spaces"][vc_column][rev_slider alias="home-6"][cm_buy_now title="Find your dream car in our inventory!" link="url:'.urlencode(get_the_permalink($cm_pages["vehicles"])).'|title:Vehicles%20inventory|"][/vc_column][/vc_row][vc_row][vc_column][cm_3features style="3"][/vc_column][/vc_row][vc_row][vc_column][cm_newest_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="1" posts_per_page="10"][/vc_column][/vc_row][vc_row][vc_column][cm_4count_box style="3" count1="387" count2="46" count3="919" count4="196"][cm_buy_now_with_shortdescription link="url:'.urlencode(get_the_permalink($cm_pages["add_vehicle"])).'|title:Post%20ad|"][/vc_column][/vc_row][vc_row full_width="stretch_row" css=".vc_custom_1484204801794{background-color: #f7f7f7 !important;}"][vc_column][cm_popular_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="1" scrollstyle="1" speed="1" posts_per_page="10"][/vc_column][/vc_row][vc_row][vc_column][cm_news_and_rumors style="1"][/vc_column][/vc_row][vc_row css=".vc_custom_1484204868544{background-color: #f7f7f7 !important;}"][vc_column][cm_team_memebers per_row="4" posts_per_page="4"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_makes jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" style="1" scrollstyle="1" posts_per_page="10" speed="1" heading="Vehicle Makes" description="Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer"][/vc_column][/vc_row]';
                $my_post = array(
                    "ID"           => $cm_pages["home4"],
                    "post_content" => $new_page_content,
                );
                ...
                wp_update_post( $my_post );
                //Home 6
                $new_page_content = '[vc_row][vc_column][cm_vehicle_search jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" style="2"][/vc_column][/vc_row][vc_row][vc_column][cm_3features style="2"][/vc_column][/vc_row][vc_row][vc_column][cm_vehicle_types jsvehiclemanagerpageid="'.$cm_pages["vehicles"].'" posts_per_page="15" scrollstyle="2" speed="1" heading="Vehicle by types" description="Vehicle types are listed here"][/vc_column][/vc_row][vc_row][vc_column][cm_newest_vehicles jsvehiclemanagerpageid="'.$cm_pages["vehicledetail"].'" style="2" posts_per_page="20" icon="fa fa-credit-card-alt"][/vc_column][/vc_row][vc_row][vc_column][cm_news_and_rumors style="1"][/vc_column][/vc_row][vc_row][vc_column][cm_image_and_text_box image="973"][/vc_column][/vc_row]';
                $my_post = array(
                    "ID"           => $cm_pages["home6"],
                    "post_content" => $new_page_content,
                );
                ...
                $wp_page_array = array(
                                    ...
                                    "vehicledetail" => $cm_pages["vehicledetail"],
                ...
                // ----------------Custom posts -------- //
                // News & Rumors
                $new_page_title = "NEW CARS FOR 2017";
                ...
                $new_page_title = "PORCHE RELEASE NEW MODEL";
                ...
                $new_page_title = "BEST NEW CARS FOR 2017";
                ...
                __update_post_meta($new_page_id, "cm_price" , "$ 1500");
                __update_post_meta($new_page_id, "cm_line1" , "Unlimited domains");
                __update_post_meta($new_page_id, "cm_line2" , "Unlimited pages website");
                __update_post_meta($new_page_id, "cm_line3" , "Custom contact and signup form");
                __update_post_meta($new_page_id, "cm_line4" , "select from our 300 themes");
                __update_post_meta($new_page_id, "cm_buynowlink" , "#");
                ...
                $new_page_id = wp_insert_post($new_page);
                update_post_meta($new_page_id, "_wp_page_template", $new_page_template);
                __update_post_meta($new_page_id, "cm_price" , "$ 750");
                __update_post_meta($new_page_id, "cm_line1" , "Domain name selection");
                __update_post_meta($new_page_id, "cm_line2" , "8 pages for your webiste");
                __update_post_meta($new_page_id, "cm_line3" , "custom contact form");
                __update_post_meta($new_page_id, "cm_line4" , "select from our 30 themes");
                __update_post_meta($new_page_id, "cm_buynowlink" , "#");
                ...
                $new_page_id = wp_insert_post($new_page);
                update_post_meta($new_page_id, "_wp_page_template", $new_page_template);
                __update_post_meta($new_page_id, "cm_price" , "$ 500");
                __update_post_meta($new_page_id, "cm_line1" , "Domain name selection");
                __update_post_meta($new_page_id, "cm_line2" , "5 pages for your website");
                __update_post_meta($new_page_id, "cm_line3" , "custom contact");
                __update_post_meta($new_page_id, "cm_line4" , "select from our 20 themes");
                __update_post_meta($new_page_id, "cm_buynowlink" , "#");
                ...
                if( !$menu_exists){
                    ...
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["home"],
                    ...
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["home1"],
                    ...
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["home2"],
                        "menu-item-parent-id" => $parent_home,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["home3"],
                        "menu-item-parent-id" => $parent_home,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["home4"],
                        "menu-item-parent-id" => $parent_home,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["home5"],
                        "menu-item-parent-id" => $parent_home,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["home6"],
                        "menu-item-parent-id" => $parent_home,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    // Vehicles
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["vehicles"],
                        "menu-item-parent-id" => 0,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                      );
                    $parent_vehicles = wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["vehicles_by_city"],
                        "menu-item-parent-id" => $parent_vehicles,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                      );
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["vehicles_by_make"],
                        "menu-item-parent-id" => $parent_vehicles,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["vehicles_by_type"],
                        "menu-item-parent-id" => $parent_vehicles,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["sellers_by_city"],
                        "menu-item-parent-id" => $parent_vehicles,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["seller_list"],
                        "menu-item-parent-id" => $parent_vehicles,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["search_vehicles"],
                        "menu-item-parent-id" => 0,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["my_profile"],
                        "menu-item-parent-id" => 0,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                      );
                    $parent_profile = wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["add_vehicle"],
                        "menu-item-parent-id" => $parent_profile,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                      );
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["my_vehicles"],
                        "menu-item-parent-id" => $parent_profile,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    $itemData =  array(
                        ...
                        "menu-item-object-id" => $cm_pages["blog"],
                        "menu-item-parent-id" => 0,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["news_and_rumors"],
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["faq"],
                        "menu-item-parent-id" => $parent_pages,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["pricing_table"],
                        "menu-item-parent-id" => $parent_pages,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["thank_you"],
                        "menu-item-parent-id" => $parent_pages,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["ourteam"],
                        "menu-item-parent-id" => 0,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                    ...
                    wp_update_nav_menu_item($menu_id, 0, $itemData);
                    $itemData =  array(
                        "menu-item-object-id" => $cm_pages["contact_us"],
                        "menu-item-parent-id" => 0,
                        "menu-item-object" => "page",
                        "menu-item-type"      => "post_type",
                        "menu-item-status"    => "publish"
                ...
                $widget_cm_footerusefullinks_array[1] = array(
                    ...
                    "title4"=>"Shortlisted", "link4"=> get_the_permalink($cm_pages["vehicles"]), 
                    "title5"=>"Seller List", "link5"=> get_the_permalink($cm_pages["seller_list"]), 
                    "title6"=>"", "link6"=> "#", 
                    "title7"=>"", "link7"=> "#", 
                    "title8"=>"", "link8"=> "#", 
                    "title9"=>"", "link9"=> "#", 
